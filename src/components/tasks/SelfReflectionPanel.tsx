  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-sm font-semibold text-primary font-display">Self reflection</h2>
          <p className="text-xs text-muted">Your checklist for the day — fully customizable</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            type="button"
            onClick={() => { setEditing(null); setShowFieldForm(true); }}
      ...
      {fields.length === 0 ? (
        <EmptyState
          icon={Sparkles}
          title="Build your reflection list"
          description="Add things like “Five prayers”, “10 pages of books”, mood, or anything you want to track daily."
          action={
      ...
      {deletingId && (
        <ConfirmDialog
          ...
          message="It will disappear from your template. Past answers for this field are cleared."
          confirmLabel="Remove"
          onConfirm={() => {
    ...
    field.type === 'checkbox' ? 'Checkbox'
      : field.type === 'number'
        ? [
            ...
            field.unit || null,
          ].filter(Boolean).join(' · ') || 'Number'
        : field.type === 'textarea' ? 'Long text' : 'Text';
...
function FieldFormModal({
  ...
  return (
    <Modal title={field ? 'Edit reflection item' : 'New reflection item'} onClose={onClose}>
      <form
        onSubmit={e => {
          e.preventDefault();
